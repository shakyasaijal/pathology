<?php
    ...
    function sendConsultEmail($subject, $email_template, $email_to, $user_name, $data){
        //Create an instance; passing `true` enables exceptions
        $mail = new PHPMailer(true);
        ...
        try {
            ...
            $mail->setFrom(EMAIL_USERNAME, 'Breast Cancer Pathology');
            $mail->addAddress($email_to, $user_name);     //Add a recipient
            $mail->addBcc($data['patient_email']);     //Add a bcc
            ...
            $html_content = file_get_contents('../app/Views/base/email_templates/'.$email_template);
            $html_content = str_replace('{{time}}', $data['time'], $html_content);
            $html_content = str_replace('{{patient_name}}', $data['patient_name'], $html_content);
            $html_content = str_replace('{{patient_email}}', $data['patient_email'], $html_content);
            $html_content = str_replace('{{doctor_name}}', $data['doctor_name'], $html_content);
            $mail->Body = $html_content;
            $mail->send();
